...
Oryx is built for the reading case: the file you want to open fast, enjoy reading, maybe export to PDF and close, without an editor or a browser tab in the way. It renders beautiful markdown natively in one small binary, opens files in a fraction of a second whatever their size, and ships with many themes and nice-looking PDF export.

- A normal document is on screen in well under 100 milliseconds from cold, and an 8MB file takes under half a second.
- Oryx has no panes, no toolbars and no menus. F1 lists the shortcuts, Escape closes whatever is open.
- It renders beautiful markdown and PDF exports with its 31 themes that address 51 color roles.
- One binary and a folder of themes. No browser engine, no runtime, no GPU requirement.
- It performs the same on a new laptop as on an old machine with no graphics card.
...
- **Basic HTML**: Oryx handles the common subset: centered blocks, images at a set width or height, rows of clickable badges, line breaks, and the inline tags down to sub and sup.

![Oryx rendering a GitHub style README](screenshots/github.png)

## Tools

- **Find in document**: Ctrl+F searches text. The search is smart about case. `oryx` will match Oryx, ORYX and oryx. `Oryx` will perform an exact match. A match can cross styling, so `fast viewer` is found even when it was written as **fast** *viewer*.
...
- **Ctrl+O**: Opens the native file dialog.
- **F5**: Reloads a file being edited elsewhere. 
- **Zoom**: Using Ctrl+Plus (in) and Ctrl+Minus (out).
- **Persistence**: Window geometry, the active theme, the sidebar and the last folder all saved and restored at every start.
...
On Ctrl+T the browser previews themes and applies them live.
...
Ctrl+E exports the document to a PDF and asks where to save it. The page carries the document as it looks on screen with the configured export theme. Headings are the outline a PDF viewer navigates by, and the fonts are embedded.
...
Measured on my 2019 Linux laptop, release build. First frame is cold launch to the first paint; the export column is Ctrl+E to the finished PDF:
...
- Remote images ride the operating system's TLS stack, which on Linux means that it needs the OpenSSL library (which is normally shipped with every distro). Without it, badges show placeholders but everything else works.
